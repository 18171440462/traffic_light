...
def decode_bbox(pred_hms, pred_whs, pred_offsets, threshold, cuda, topk=100):
    ...
    for batch in range(b):
        ...
        #-------------------------------------------------------------------------#
        heat_map    = pred_hms[batch].permute(1,2,0).contiguous().view([-1,c])
        pred_wh     = pred_whs[batch].permute(1,2,0).contiguous().view([-1,2])
        pred_offset = pred_offsets[batch].permute(1,2,0).contiguous().view([-1,2])
